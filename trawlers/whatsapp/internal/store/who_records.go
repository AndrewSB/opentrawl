...
func (s *Store) whoCandidateRecords(ctx context.Context) ([]whoCandidateRecord, error) {
	return s.readWhoCandidateRecords(ctx, true, true)
}

func (s *Store) whoCandidateRecordsWithoutNameMerge(ctx context.Context) ([]whoCandidateRecord, error) {
	return s.readWhoCandidateRecords(ctx, false, true)
}

// whoCandidateNameRecords answers "what is this participant called", which is
// all the sender-name map reads. It skips the per-message statistics, which
// that map never looks at and which cost an order of magnitude more than
// everything else in this path: they attribute every message to every
...
func (s *Store) whoCandidateNameRecords(ctx context.Context) ([]whoCandidateRecord, error) {
	return s.readWhoCandidateRecords(ctx, false, false)
}

func (s *Store) readWhoCandidateRecords(ctx context.Context, mergeSameName, withStats bool) ([]whoCandidateRecord, error) {
