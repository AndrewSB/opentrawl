...
`scripts/check-linux` builds and vets every Go module for Linux, and CI runs it
on every push. It does not run the test suites: several need macOS fixtures, so
a green Linux gate means the tree still compiles for Linux, which is the part
